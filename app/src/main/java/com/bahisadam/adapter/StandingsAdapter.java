...
public class StandingsAdapter extends SectionedAdapter<StandingsAdapter.StandingVH> implements Constant {
    ...
    public StandingsAdapter(Activity activity) {
        this.mData = new ArrayList<Group>();
    ...
    public void add(List<Group> sublist){
    ...
    public void onBindViewHolder(StandingVH itemHolder, int section, int relativePosition, int absolutePosition) {
        ...
        if(standing != null) {
            ...
            holder.teamName.setText(Utilities.abbreviateTeamName(team));
            holder.rank.setText(""+(relativePosition+1));
            if(standing.standingGroup != null) {
                switch (standing.standingGroup) {
                    case "promotion" :
                        holder.rank.setBackground(activity.getResources().getDrawable(R.drawable.shape_uefaofleauge));
                        holder.rank.setTextColor(Color.parseColor("#ffffff"));
                        break;
                    case "relegation" :
                        holder.rank.setBackground(activity.getResources().getDrawable(R.drawable.shape_lastofleauge));
                        holder.rank.setTextColor(Color.parseColor("#ffffff"));
                        break;
                    case "relegation-1" :
                        holder.rank.setBackground(activity.getResources().getDrawable(R.drawable.shape_lastofleauge));
                        holder.rank.setTextColor(Color.parseColor("#666666"));
                        break;
                    case "promotion-1" :
                        holder.rank.setBackground(activity.getResources().getDrawable(R.drawable.shape_firstofleauge));
                        holder.rank.setTextColor(Color.parseColor("#ffffff"));
                        break;
                    case "promotion-2" :
                        holder.rank.setBackground(activity.getResources().getDrawable(R.drawable.shape_firstofleauge));
            ...
            Crashlytics.logException(new Exception("team not found"));
        }
    }
