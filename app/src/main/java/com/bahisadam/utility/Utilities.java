...
public class Utilities implements Constant {
    ...
    // Set Font TypeFace
    public static void fontAwesome(Context ctx, TextView tv, Integer icon) {
    ...
    public static RestClient buildRetrofit(boolean saveCookie) {
        ...
        if (saveCookie) {
            builder.addNetworkInterceptor(new ReceivedCookiesInterceptor());
        }
        builder.addInterceptor(new LoggingInterceptor.Builder().loggable(BuildConfig.DEBUG).build());
...
//        }
    ...
    public static void showSnackBarInternet(final Context ctx, View view, String message) {
        Snackbar snackbar = Snackbar
                .make(view, message, Snackbar.LENGTH_SHORT)
                .setAction(ctx.getString(R.string.retry), new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        // Routing to connection settings
...
//                .negativeText(ctx.getString(R.string.cancel))
    ...
    public static String getString(Context ctx, String string) {
    ...
    public static List<ToolbarItem> createToolbarItems(Context ctx) {
        ...
        ToolbarItem news = new ToolbarItem();
        news.height = 20;
        news.width = 20;
        news.img = "news";
        news.name = "Haberler";
        toolbarItems.add(news);
    ...
    public static void openPlayerDetails(Activity activity, String player_id) {
        Intent intent = new Intent(activity, HomeActivity.class);
        intent.putExtra(BaseActivity.PAGE, BaseActivity.RESULT_LOAD_PLAYER);
        intent.putExtra(BaseActivity.PLAYER_ID, player_id);
        startActivity(activity, intent);
    }

    public static void openPlayerDetails(Activity activity, String player_id, String name) {
        Intent intent = new Intent(activity, HomeActivity.class);
        intent.putExtra(BaseActivity.PAGE, BaseActivity.RESULT_LOAD_PLAYER);
        intent.putExtra(BaseActivity.PLAYER_ID, player_id);
        intent.putExtra(BaseActivity.PLAYER, name);
        startActivity(activity, intent);
    }
    ...
    public static void openUserPredictions(Activity activity, String userId) {
        Intent intent = new Intent(activity, PredictionUserActivity.class);
        intent.putExtra(BaseActivity.ID, userId);
        startActivity(activity, intent);
    }

    public static void openNewsDetailActivity(Activity activity, String newsId) {
        Intent intent = new Intent(activity, NewsDetailActivity.class);
        intent.putExtra(BaseActivity.ID, newsId);
        startActivity(activity, intent);
    }
    ...
    public static Bitmap getBitmapCountry(int id, Context context) {
        ...
        if (bitmap == null) {
            try {
                ...
                Bitmap bmp = Bitmap.createBitmap(160,
    ...
    public static void paintFavoriteIcon(Activity activity, TextView icon, boolean active) {
        ...
        if (active) {
            ...
            fontAwesome(activity, icon, R.string.fa_star);
        } else {
            fontAwesome(activity, icon, R.string.fa_star_o);
        }

        icon.setTextColor(color);
    }

    public static void paintFavoriteIconOntoWhiteBg(Activity activity, TextView icon,boolean active){
    ...
    public static void addRemoveMatchToFavorites(String match_id, boolean remove, Context ctx){
        RestClient client = Utilities.buildRetrofit();
        Call<AddRemoveFavoriteRequest.Response> call;

        AddRemoveFavoriteRequest request = new AddRemoveFavoriteRequest();
        request.device_id = Utilities.getDeviceId(ctx);
        request.match_id = match_id;
        request.remove = remove;
        request.type = "match";

        call = client.addRemoveFavorite(request);
    ...
    public static void addRemoveTeamToFavorites(int team_id, boolean remove, Context ctx){
        RestClient client = Utilities.buildRetrofit();
        Call<AddRemoveFavoriteRequest.Response> call;
    ...
    public static String abbreviateTeamName(String team) {
        if(team != null && team.contains(" ")) {
            String[] names = team.trim().split(" ");
            if(names.length > 1 && names[1].length() > 3) {
                return names[0] + " " + names[1].substring(0, 3) + ".";
            }
        }
